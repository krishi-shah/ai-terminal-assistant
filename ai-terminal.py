#!/usr/bin/env python3
"""
AI Terminal Assistant - Professional Edition
Works with any OpenAI model on Windows, Linux, and macOS

Installation:
1. pip install openai python-dotenv colorama
2. Create .env file with: OPENAI_API_KEY=your-key-here
3. python ai-terminal.py
...
try:
    ...
    print("Install it with: pip install openai python-dotenv colorama")
    print("Or try: pip3 install --user openai python-dotenv colorama")
    sys.exit(1)
...
try:
    ...
    pass  # Colors may not work properly on Windows without colorama
...
# Detect shell type for Linux/Unix systems
def get_shell_type():
    ...
    if 'bash' in shell:
        ...
        return "bash"  # Default to bash for Unix systems
...
def print_banner():
    """Welcome message"""
    print(f"\n{C.C}{'='*60}")
    print(f"  AI Terminal Assistant")
    print(f"  Natural Language to {OS_NAME} Commands")
    if CURRENT_OS != "Windows":
...
def generate_command(client, model, user_input):
    ...
    # Determine shell type based on OS and detected shell
    if CURRENT_OS == "Windows":
        shell_type = "PowerShell/CMD"
        shell_rules = "Use common Windows commands (PowerShell preferred, CMD compatible). Make it safe. One line command."
    else:
        shell_type = SHELL_TYPE
        shell_rules = f"Use common Unix/Linux tools optimized for {SHELL_TYPE}. Use GNU coreutils, grep, sed, awk, find, etc. Make it safe and efficient. One line command."
    
    prompt = f"""Convert this request into a {OS_NAME} {shell_type} command. Respond ONLY with valid JSON, no markdown:

Request: "{user_input}"

{{
  "command": "the {shell_type} command",
...
Rules: {shell_rules}"""
    ...
    try:
        system_content = f"You are a {OS_NAME} command line expert. Always return valid JSON only."
